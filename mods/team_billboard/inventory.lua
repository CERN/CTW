...
local function chats(str)
	minetest.log("action", "[team_billboard] "..str)
	minetest.chat_send_all("[team_billboard] "..str)
end

local function check_team(player, tname)
	local team = teams.get_by_player(player)
	chats("check team "..tname..(team and team.name or "noteam"))
				...
				if listname == "ideas" then
					...
					if minetest.get_item_group(stack:get_name(), "ctw_idea") > 0 then
						...
						if idea_id then
							...
							chats("result "..(succ and "success" or err))
							-- clear the just added stack
							inv:set_stack(listname, index, "")
							-- check if we need to add the idea
							if not inv:contains_item(listname, stack:get_name()) then
								chats("noexist adding")
								inv:add_item(listname, stack:get_name())
							end
						end
					end
				elseif listname == "approvals" then
	...
	if not inv then
		...
		minetest.create_detached_inventory("team_billboard_"..team.name, {
			allow_move = function(inv, from_list, from_index, to_list, to_index, count, player)
				return inv_callbacks.allow_move(tname, inv, from_list, from_index, to_list, to_index, count, player)
			end,

			allow_put = function(inv, listname, index, stack, player)
				return inv_callbacks.allow_put(tname, inv, listname, index, stack, player)
			end,

			allow_take = function(inv, listname, index, stack, player)
				return inv_callbacks.allow_take(tname, inv, listname, index, stack, player)
			end,

			on_put = function(inv, listname, index, stack, player)
				return inv_callbacks.on_put(tname, inv, listname, index, stack, player)
			end,
		})
	end
